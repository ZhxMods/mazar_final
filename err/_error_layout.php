...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn-primary { background:#fff; color:#1d4ed8; font-weight:700; padding:.78rem 1.8rem; border-radius:.875rem; display:inline-flex; align-items:center; gap:.5rem; transition:transform .2s,box-shadow .2s,background .15s; text-decoration:none; font-size:.875rem; white-space:nowrap; box-shadow:0 4px 20px rgba(0,0,0,.2); }
    .btn-primary:hover { transform:translateY(-2px); box-shadow:0 12px 32px rgba(0,0,0,.3); background:#eff6ff; }
    .btn-secondary { border:1.5px solid rgba(255,255,255,.2); color:rgba(255,255,255,.85); font-weight:600; padding:.75rem 1.6rem; border-radius:.875rem; display:inline-flex; align-items:center; gap:.5rem; transition:background .15s,border-color .15s,transform .2s; text-decoration:none; font-size:.875rem; white-space:nowrap; }
...
<main class="hero-bg grid-overlay flex-1 flex flex-col relative overflow-hidden">
  ...
  <!-- Blobs -->
  <div class="blob w-96 h-96 bg-blue-700 opacity-20 top-0 right-0" style="width:500px;height:500px;top:-120px;right:-80px"></div>
  <div class="blob bg-indigo-500 opacity-10 bottom-0 left-0" style="width:350px;height:350px;bottom:-80px;left:-60px"></div>

  <!-- TOP NAV -->
  <nav class="relative z-20 w-full px-5 py-4 slide-up flex items-center justify-between" style="border-bottom:1px solid rgba(255,255,255,.06)">
    <a href="https://mazar.zya.me/" class="flex items-center gap-2.5">
      <div class="w-9 h-9 rounded-xl flex items-center justify-center" style="background:linear-gradient(135deg,#2563eb,#1d4ed8);box-shadow:0 0 20px rgba(37,99,235,.4)">
        <i data-lucide="graduation-cap" class="w-5 h-5 text-white"></i>
      </div>
      <div>
        <div class="text-white font-black text-base tracking-tight leading-none">MAZAR</div>
        <div class="text-xs font-semibold tracking-widest uppercase leading-none mt-0.5" style="color:rgba(96,165,250,.7)">Éducation · Maroc</div>
      </div>
    </a>
    <div class="hidden md:flex items-center gap-1">
      <a href="https://mazar.zya.me/" class="nav-link"><i data-lucide="home" class="w-3.5 h-3.5"></i> Accueil</a>
      <a href="https://mazar.zya.me/student/dashboard.php" class="nav-link"><i data-lucide="layout-dashboard" class="w-3.5 h-3.5"></i> Tableau de bord</a>
      <a href="https://mazar.zya.me/student/mazar-ai.php" class="nav-link"><i data-lucide="sparkles" class="w-3.5 h-3.5"></i> MAZAR AI</a>
    </div>
    <div class="flex items-center gap-2">
      <a href="https://mazar.zya.me/login.php" class="hidden sm:inline-flex items-center gap-1.5 text-sm font-semibold transition-colors" style="color:rgba(255,255,255,.6)">
        <i data-lucide="log-in" class="w-3.5 h-3.5"></i> Connexion
      </a>
      <a href="https://mazar.zya.me/register.php" class="inline-flex items-center gap-1.5 font-bold text-blue-700" style="background:#fff;padding:.42rem 1rem;border-radius:.6rem;font-size:.8rem;text-decoration:none">
        S'inscrire
      </a>
    </div>
  </nav>
    ...
    <div class="w-full max-w-2xl">
      ...
      <div class="slide-up flex flex-wrap items-center justify-center gap-2 mb-6">
        ...
        <div class="status-badge <?= $badgeClass ?>">
          <div class="w-1.5 h-1.5 rounded-full bg-current" style="box-shadow:0 0 6px currentColor"></div>
          HTTP <?= $errorCode ?>
        </div>
        <div class="detail-pill"><i data-lucide="clock" class="w-2.5 h-2.5"></i> <?= date('d/m/Y · H:i:s') ?></div>
        <div class="detail-pill"><i data-lucide="hash" class="w-2.5 h-2.5"></i> ERR-<?= strtoupper(substr(md5(microtime()), 0, 8)) ?></div>
      </div>
      ...
      <div class="slide-up slide-up-d2 text-center mb-6">
        ...
        <p class="text-sm mt-3 flex items-center justify-center gap-1.5" style="color:rgba(147,197,253,.7)">
          <i data-lucide="lightbulb" class="w-3.5 h-3.5 flex-shrink-0" style="color:rgba(251,191,36,.7)"></i>
      ...
      <div class="slide-up slide-up-d2 mb-6">
        ...
        <form action="https://mazar.zya.me/search.php" method="GET" class="search-box">
          <i data-lucide="search" class="w-4 h-4 flex-shrink-0" style="color:rgba(255,255,255,.35)"></i>
          <input type="text" name="q" placeholder="Ex : tableau de bord, cours, IA…" autocomplete="off">
          <button type="submit"><i data-lucide="arrow-right" class="w-4 h-4 text-white"></i></button>
        </form>
      </div>
      <?php endif; ?>
      ...
      <div class="slide-up slide-up-d3 flex flex-col sm:flex-row items-center justify-center gap-3 mb-8">
        <?php if (!empty($showLogin)): ?>
          <a href="https://mazar.zya.me/login.php" class="btn-primary"><i data-lucide="log-in" class="w-4 h-4"></i> Se connecter</a>
          <a href="https://mazar.zya.me/register.php" class="btn-secondary"><i data-lucide="user-plus" class="w-4 h-4"></i> Créer un compte</a>
          <a href="https://mazar.zya.me/" class="btn-secondary"><i data-lucide="home" class="w-4 h-4"></i> Accueil</a>
        <?php elseif (!empty($isMaintenance)): ?>
          <button onclick="window.location.reload()" class="btn-primary"><i data-lucide="refresh-cw" class="w-4 h-4"></i> Actualiser la page</button>
        <?php else: ?>
          <a href="https://mazar.zya.me/" class="btn-primary"><i data-lucide="home" class="w-4 h-4"></i> Retour à l'accueil</a>
          <a href="javascript:history.back()" class="btn-secondary"><i data-lucide="arrow-left" class="w-4 h-4"></i> Page précédente</a>
        <?php endif; ?>
      </div>
      ...
      <div class="slide-up slide-up-d4">
        ...
        <div class="flex flex-wrap items-center justify-center gap-2">
          <a href="https://mazar.zya.me/student/dashboard.php" class="quick-link"><i data-lucide="layout-dashboard" class="w-3 h-3"></i> Tableau de bord</a>
          <a href="https://mazar.zya.me/student/mazar-ai.php" class="quick-link"><i data-lucide="sparkles" class="w-3 h-3"></i> MAZAR AI</a>
          <a href="https://mazar.zya.me/student/courses.php" class="quick-link"><i data-lucide="book-open" class="w-3 h-3"></i> Mes cours</a>
          <a href="https://mazar.zya.me/login.php" class="quick-link"><i data-lucide="log-in" class="w-3 h-3"></i> Connexion</a>
          <a href="https://mazar.zya.me/register.php" class="quick-link"><i data-lucide="user-plus" class="w-3 h-3"></i> S'inscrire</a>
          <a href="mailto:support@mazar.ma" class="quick-link"><i data-lucide="mail" class="w-3 h-3"></i> Support</a>
        </div>
      </div>
      ...
      <div class="slide-up slide-up-d4 mt-6 grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div class="glass-card p-4 text-center">
          <i data-lucide="refresh-cw" class="w-5 h-5 mx-auto mb-2" style="color:rgba(147,197,253,.7)"></i>
        ...
        <div class="glass-card p-4 text-center">
          <i data-lucide="wifi" class="w-5 h-5 mx-auto mb-2" style="color:rgba(147,197,253,.7)"></i>
        ...
        <div class="glass-card p-4 text-center">
          <i data-lucide="headphones" class="w-5 h-5 mx-auto mb-2" style="color:rgba(147,197,253,.7)"></i>
...
<footer class="relative z-10 py-5 px-6">
  <div class="max-w-5xl mx-auto">
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mb-4">
      <div class="flex items-center gap-2">
        <div class="w-7 h-7 rounded-lg flex items-center justify-center" style="background:rgba(37,99,235,.25);border:1px solid rgba(37,99,235,.3)">
          <i data-lucide="graduation-cap" class="w-3.5 h-3.5 text-blue-300"></i>
        </div>
        <span class="text-sm font-bold" style="color:rgba(255,255,255,.6)">MAZAR</span>
        <span class="text-xs" style="color:rgba(255,255,255,.2)">— La plateforme éducative numéro 1 au Maroc</span>
      </div>
      <div class="flex items-center gap-3">
        <a href="mailto:support@mazar.ma" class="text-xs font-medium transition-colors flex items-center gap-1" style="color:rgba(255,255,255,.3)">
          <i data-lucide="mail" class="w-3.5 h-3.5"></i> support@mazar.ma
        </a>
        <span style="color:rgba(255,255,255,.1)">|</span>
        <a href="https://mazar.zya.me/privacy.php" class="text-xs transition-colors" style="color:rgba(255,255,255,.3)">Confidentialité</a>
        <a href="https://mazar.zya.me/terms.php" class="text-xs transition-colors" style="color:rgba(255,255,255,.3)">CGU</a>
      </div>
    </div>
